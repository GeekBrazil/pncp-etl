...
def coleta_imobiliaria(conn, dom, creci=None, nome=None, cidade=None, uf=None, max_anuncios=20):
    ...
    if not home:
        ...
        return 0
    creci = creci or (re.search(r"creci[:\s]*([\d.\-A-Za-z/]+)", home, re.I) or [None, None])[1]
    ...
    # anúncios: links de detalhe na home
    urls = list(dict.fromkeys(re.findall(r'href="(https?://[^"]*(?:detalhes|imovel|imoveis)/[^"]+\.html?)"', home)))[:max_anuncios]
    gravados = 0
    for u in urls:
        page = _get(u)
        if page:
